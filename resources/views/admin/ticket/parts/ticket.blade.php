<form action="{{ $url }}" method="POST" enctype="multipart/form-data" id="ticketForm">
    ...
    <div class="row mb-4 align-items-center">
        ...
        <div class="col-md-4">
            <select class="form-select ticket-notice addSelect2 w-auto" name="ticket_notice" id="ticket_notice">
                <option value="alert_all" {{ $ticket?->ticket_notice == 'alert_all' ? 'selected' : '' }}>Alert All
                </option>
                <option value="alert_to_user" {{ $ticket?->ticket_notice == 'alert_to_user' ? 'selected' : '' }}>Alert To
                    User</option>
                <option value="do_not_send_alert" {{ $ticket?->ticket_notice == 'do_not_send_alert' ? 'selected' : '' }}>
                    -- Do Not Send Alert -- </option>
            </select>
        </div>
    </div>
    ...
    <div class="row mb-3 align-items-center">
        ...
        <div class="col-md-10 d-flex align-items-center gap-2 flex-wrap">
            <input name="due_date" type="date" class="form-control w-auto"
                value="{{ !empty($ticket?->due_date) ? \Carbon\Carbon::parse($ticket?->due_date)->format('Y-m-d') : now()->format('Y-m-d') }}">
        </div>
    </div>
            ...
            <select name="assign_id" id="assign-to-select" class="form-select w-auto">
                ...
                @if (isset($developers))
                    @foreach ($developers as $developer)
                        <option value="{{ $developer->id }}" {{ $developer->id == $ticket?->assign_id ? 'selected' : '' }}>
                            {{ $developer->name . ' - ' . $developer?->role?->name }}</option>
                    @endforeach
                @endif
            </select>
        </div>
    ...
    <div class="row mb-3 align-items-center">
        ...
        <div class="col-md-4">
            <select class="form-select addSelect2" name="priority" id="priority">
                ...
                <option value="Low" {{ old('priority', $ticket?->priority ?? 'Low') == 'Low' ? 'selected' : '' }}>
                    Low
                </option>
            </select>
            @error('priority')
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            <div class="mb-3">
                <label class="form-label" for="ticket-description-input-body-hidden">Body <span
                        class="text-danger">*</span></label>

                <input type="hidden" name="body" id="ticket-description-input-body-hidden"
                    value="{{ old('body', $ticket?->body ?? '') }}">

                <div class="ticket-description-editor-body" id="ticket-description-editor-body" style="height: 200px;">
                    {!! old('body', $ticket?->body ?? '') !!}</div>
    ...
    <div class="row mb-3 align-items-center">
        ...
        <div class="col-md-6 d-flex align-items-center gap-3">
            <select class="form-select addSelect2 w-auto" data-placeholder="-- Select a canned response --"
                ...
                <option value="sample" {{ $ticket?->canned_response == 'sample' ? 'selected' : '' }}>Sample (with
                    variables)</option>
                <option value="os_ticket" {{ $ticket?->canned_response == 'os_ticket' ? 'selected' : '' }}>What is
                    osTicket (sample)?</option>
            </select>
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            <div class="mb-3">
                <label class="form-label" for="ticket-description-input">Response Description <span
                        class="text-danger">*</span></label>

                <input type="hidden" name="description" id="ticket-description-input"
                    value="{{ old('description', $ticket?->description ?? '') }}">

                <div class="ticket-description-editor" id="ticket-description-editor" style="height: 200px;">
                    {!! old('description', $ticket?->description ?? '') !!}</div>
    ...
    {{-- <div class="row mb-3">
        <div class="col-md-12">
            <div class="p-3 text-start border rounded bg-light"
                style="border-style: dashed !important; border-color: #ccc !important;">
    ...
    <div class="row mb-3 align-items-center">
        ...
        <div class="col-md-4">
            <select class="form-select addSelect2 w-auto" data-placeholder="-- Select a status -- " name="ticket_status"
                id="ticket-status">
                <option value=""></option>
        ...
        <div class="col-md-12">
            <div class="col-md-12">
                <div class="mb-3">
                    ...
                    <div class="ticket-internal-note-editor" id="ticket-internal-note-editor" style="height: 200px;">
                        {!! old('internal_note', $ticket?->internal_note ?? '') !!}
                    </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-lg-12 mb-3">
                            <label for="modal_user_name" class="form-label">User Name <span
                                    class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control" id="modal_user_name"
                                placeholder="Enter Name" required>
                        </div>
                        <div class="col-lg-12 mb-3">
                            <label for="modal_user_email" class="form-label">Email <span
                                    class="text-danger">*</span></label>
                            <input type="email" name="email" class="form-control" id="modal_user_email"
                                placeholder="Enter Email" required>
                        </div>
                        <div class="col-lg-12 mb-3">
                            <label for="modal_user_role" class="form-label">Role <span
                                    class="text-danger">*</span></label>
                            <select name="role_id" id="modal_user_role" class="form-select" required
                                style="width: 100%;">
                        ...
                        <div class="col-lg-12 mb-3">
                            ...
                            <select name="department_id" id="modal_user_department" class="form-select"
                                style="width: 100%;">
                                <option value="">-- Select Department --</option>

<x-master-layout>
    <x-form-wrapper action="{{ isset($action) ? $action : (isset($digitalservice) ? 'Edit Digital Service' : 'Create Digital Service') }}">
        <form id="digitalServicesForm"
            action="{{ isset($digitalservice) ? route('admin.digital.services.update', encrypt($digitalservice->id)) : route('admin.digital.services.store') }}"
            method="post"

...
@if (isset($variants) && $variants->isNotEmpty())
    @foreach ($variants as $key => $variant)
        <input {{ $is_disabled }} type="hidden" class="existing-variant-id" name="variant[{{ $key }}][variant_id]"
            value="{{ $variant->id }}">
            ...
            <div class="row mb-5 builder-rows">
                ...
                <div class="row mb-3">
                    <div class="col-md-5">
                        ...
                        <input {{ $is_disabled }} type="text" class="form-control builder-name-input"
                            name="variant[{{ $key }}][name]" placeholder="Field name" value="{{ $variant->name }}">
                    </div>

                    <div class="col-md-5">
                        <label class="form-label">Price</label>
                        <input {{ $is_disabled }} type="text" class="form-control builder-price-input"
                            name="variant[{{ $key }}][price]" placeholder="e.g. $1.00" value="{{ $variant->price }}">
                    </div>

                    <div class="col-md-2 d-flex align-items-end gap-2">
                        <button {{ $is_disabled }} type="button" class="btn btn-primary builder-add-btn">
                            <i class="ti ti-plus"></i>
                        </button>
                        <button {{ $is_disabled }} type="button" class="btn btn-soft-light builder-remove-btn">
                ...
                <div class="row mb-3">
                    <div class="col-lg-12 features-container">
                            ...
                            @foreach ($features as $subKey => $feature)
                                <div class="variant-feature-item d-flex align-items-center gap-2 mb-2">
                                    <div class="flex-grow-1">
                                        <input {{ $is_disabled }} type="text" name="variant[{{ $key }}][features][]"
                                            class="form-control" placeholder="Feature"
                                            value="{{ is_array($feature) ? ($feature['name'] ?? '') : $feature }}">
                                    </div>

                                    <button {{ $is_disabled }} type="button" class="btn btn-primary add-variant-feature-btn"
                                        title="Add Feature">
                                        <i class="ti ti-plus"></i>
                                    </button>

                                    <button {{ $is_disabled }} type="button" class="btn btn-soft-light remove-variant-feature-btn"
                        ...
                        @else
                            <div class="variant-feature-item d-flex align-items-center gap-2 mb-2">
                                <div class="flex-grow-1">
                                    <input {{ $is_disabled }} type="text" name="variant[0][features][]" class="form-control"
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <input {{ $is_disabled }} type="hidden" name="variant[{{ $key }}][description]"
                                id="variant-description-{{ $key }}" value="{{ $variant->description }}">
    ...
    <div class="builder-row-wrapper border p-3 rounded mb-4">
        <div class="row mb-5 builder-rows">
            ...
            <div class="row mb-3">
                <div class="col-md-5">
                    <label class="form-label">Field Name</label>
                    <input {{ $is_disabled }} type="text" class="form-control builder-name-input" name="variant[0][name]"
                        placeholder="Field name" value="">
                </div>

                <div class="col-md-5">
                    <label class="form-label">Price</label>
                    <input {{ $is_disabled }} type="text" class="form-control builder-price-input" name="variant[0][price]"
                        placeholder="e.g. $1.00" value="">
                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <input {{ $is_disabled }} type="hidden" name="variant[0][description]" id="variant-description-0"
                            value="">

<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-16">
        ...
        {{-- Search & Filters Form --}}
        <form action="{{ route('products.listing') }}" method="GET" class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div class="group flex flex-col rounded-2xl bg-white shadow-sm ring-1 ring-slate-900/5 overflow-hidden hover:shadow-md transition-all h-full">
                    ...
                    <div class="aspect-[4/3] w-full overflow-hidden bg-slate-100 relative">
                        <a href="{{ route('products.show', $product->slug) }}">
                            <img src="{{ $product->image ? asset($product->image) : asset('assets/img/default-product.jpg') }}"
                            ...
                            <div class="flex flex-col">
                                @if($product->on_sale && $product->price_for_sale)
                                    <div class="flex items-center gap-2">
                                        <p class="text-lg font-bold text-slate-900">${{ number_format($product->price_for_sale, 2) }}</p>
                                        <p class="text-sm font-medium text-slate-400 line-through">${{ number_format($product->price, 2) }}</p>
                                    </div>
        ...
        {{-- Pagination --}}
        {{ $digitalproducts->links('vendor.pagination.custom') }}
